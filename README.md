...
| **[Gemma 4 E4B](models/gemma-4-e4b/)** | 🐣 Incubating (unvalidated — catalogued, not yet boot-tested) | 2× 3090 | vLLM 🐣 | **Lightest Gemma 4 member** (~8B raw / ~4.5B effective, dense) · vision + audio + tools · bf16 only (no quant released yet) · tuned for **8 concurrent streams @ 64K ctx** instead of one long-context stream — huge VRAM headroom vs the rest of the family · `switch.sh --list --all` to see it, `--force` to launch |
